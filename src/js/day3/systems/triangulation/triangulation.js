...
var Delaunay = (function (exports) {
  ...
  exports.compute = function (points, cleanup) {
    ...
    for (currentIndex = 0; currentIndex < nv; currentIndex++) {
      ...
      // we'll store the triangle's indices in this array
      ...
      while (j < indices.length) {
        ...
        if (
          circles[circleId].radius > EPSILON &&
          //and if the circumcircle contains the current Point
          circleContains(circles[circleId], currentPoint)
        ) {
          //this triangle is invalid and must be removed from solution
          // we'll use its indices to rebuild 3 new triangles
      ...
      while (j < tmpIndices.length) {
        ...
        while (k < tmpIndices.length) {
          if (
            (tmpIndices[j] === tmpIndices[k] &&
              tmpIndices[j + 1] === tmpIndices[k + 1]) ||
            (tmpIndices[j + 1] === tmpIndices[k] &&
              tmpIndices[j] === tmpIndices[k + 1])
          ) {
...
var result = Delaunay.compute(points.concat(), true);
console.log(result);
